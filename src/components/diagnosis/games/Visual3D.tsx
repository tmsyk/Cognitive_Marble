...
export default function Visual3D({ onScore }: Visual3DProps) {
  ...
  const [isCorrectFirst, setIsCorrectFirst] = useState(true); // Randomize button position

  // Randomize button position on level change
  useEffect(() => {
  ...
  // Increase rotation complexity based on level
  const baseRotation = [20 + currentLevel * 10, 20 + currentLevel * 5, 0] as [number, number, number];
  const correctRotation = [110 + currentLevel * 10, -20 - currentLevel * 5, 0] as [number, number, number];
  const foilRotation = [45 + currentLevel * 15, 45, 120] as [number, number, number];
